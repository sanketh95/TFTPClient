...
int create_ack_packet_from_raw(char *, int, packet *);

/*copies the fields into the buffer*/
copy_ack_header_to_buffer(int block_num, char *buffer){
	buffer[0] = '\0';
	buffer[1] = '\4';
...
/*Some error messages*/
print_ack_wait_timeout(int block_number){
	printf("Timeout. Resending ACK with block number %d\n", block_number);
}

print_ack_send_error(int block_number){
	printf("Unable to send ACK packet with block number %d\n", block_number);
	exit(1);
}

print_ack_blck_mismatch_error(int bnum, int e_bnum){
