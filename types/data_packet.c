...
copy_data_header_to_buffer(int block_number, char *data_buffer, int data_size, char *buffer){
	int i=0;
	buffer[0] = '\0';
	buffer[1] = '\3';
...
print_data_wait_timeout(int block_number){
	printf("Timeout. resending the data packet with block number%d\n", block_number);
}

print_data_send_error(int block_number){
	printf("Unable to send data packet with block_number %d\n", block_number);
	exit(1);
}

print_data_blck_mismatch_error(int bnum, int e_bnum){
